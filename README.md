...
### 1. ビルド環境とESLintの世代固定 (2026/01/18更新)
本システムは安定稼働のため、**ESLint v8** および **TypeScript-ESLint v7** に環境を固定しています。
- **厳禁:** ESLint v9 世代の Flat Config (`eslint.config.mjs` 等) を作成しないでください。
- **コード作法:** `catch (e)` 構文などで変数を使用しない場合は `catch { ... }` と記述し、不要な無効化コメント（eslint-disable）を排除してください。
