...
- **usersテーブルにstatusとend_dateカラムを追加し、利用者のライフサイクルを管理**

## 使用方法

### 利用者登録・編集
1. サイドバーの「新規利用者追加」ボタンをクリック、または一覧から利用者を選択して「編集」をクリック
2. 必要情報を入力（氏名は必須）
3. **ステータス管理:** デフォルトは「利用中」。退去や逝去の際はステータスを変更し、現れた日付入力欄に終了日を記録

### 利用者一覧の閲覧
1. デフォルトでは「利用中」の利用者のみが表示されます
2. 画面上部のタブ（「すべて表示」「解約」「逝去」）を切り替えることで、過去の利用者情報を参照できます
