...
`MAINTENANCE.md`（保守管理ドキュメント）に刻むべき、今回のエクスポート機能における抜本的な改善内容を整理しました。

この記録は、10年後のエンジニアが「なぜこの設計になっているのか」を理解し、迷いなくシステムを拡張するための貴重な地図となります。
...
### 1. 修正の目的と背景
従来のExcel出力は、あらかじめ用意された全選択肢に対して「チェック（✔）」を入れる「アナログ帳票模倣型」でした。これを、**「選択された項目のみを日本語ラベルとして連結し、一つの文章として出力する」**データ駆動型の設計へ転換しました。これにより、情報の視認性が劇的に向上し、Excelテンプレートの保守コストが削減されました。

### 2. 導入された新アーキテクチャ：集計エンジン（Aggregator）
複雑な文字列生成ロジックをAPI本体（`route.ts`）から分離し、独立した集計エンジンを新設しました。

*   **関連ファイル:** `src/utils/consultation-aggregator.ts`
*   **設計の利点:**
    *   **責務の分離:** `route.ts` は「通信とファイル操作」に、`aggregator.ts` は「データの加工ロジック」に専念。
    *   **再利用性:** 相談票以外の帳票や、画面上のサマリー表示などでも同じ連結ロジックを共有可能。
    *   **テストの容易性:** UIやDB接続なしで、純粋なデータ加工ロジックのみをテスト可能。

### 3. 動的連結の対象グループ
以下の4つのセクションにおいて、動的連結リスト方式を同時適用しました。

| 対象セクション | プレースホルダ | 連結の仕様 |
| :--- | :--- | :--- |
| **相談ルート** | `{{selected_routes}}` | `家族(長男)` のように詳細入力を括弧書きで結合。 |
| **属性** | `{{selected_attributes}}` | 13項目のフラグを優先順位に基づき連結。 |
| **支援サービス** | `{{selected_services}}` | デイ、訪看等の利用状況を連結。 |
| **世帯構成** | `{{selected_households}}` | `その他(友人)` 等の詳細入力を含めて連結。 |
...
4.  **順序の固定:**
    アグリゲーター内の定数配列（`ROUTE_DEFINITIONS` 等）に基づいてループ処理を行うことで、DBからの取得順に左右されず、常に人間が読みやすい一定の順序で出力されることを保証しています。

### 5. 未来のエンジニアへの申し送り

*   **項目の追加手順:**
    1.  `src/utils/consultation-aggregator.ts` 内の定義配列に、新しいキーとラベルを追加する。
    2.  必要であれば、Excelテンプレートの表示エリアを調整する。
*   **Excelの書式設定:**
    連結文字列は長くなる可能性があるため、統合プレースホルダを配置したセルには必ず **「折り返して全体を表示する」** 設定を適用してください。
*   **非推奨コードの扱い:**
    `route.ts` 内の個別項目置換ロジックには `// Deprecated` の注釈を入れています。これらは旧テンプレート維持のために必要なため、完全にリプレースが完了するまで削除しないでください。
