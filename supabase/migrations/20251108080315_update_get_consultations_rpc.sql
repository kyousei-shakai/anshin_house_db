-- This migration updates the 'get_consultations_with_next_action' function (Version 5: Final Fix).
-- It corrects the column order in both the RETURNS TABLE and SELECT clauses to exactly match
-- the physical table structure defined in the latest `database.ts` type definitions.
--
-- このマイグレーションは 'get_consultations_with_next_action' 関数を更新します (バージョン5: 最終修正版)。
-- RETURNS TABLE句とSELECT句の両方でカラムの順序を修正し、最新の `database.ts` の
-- 型定義に記載されている物理的なテーブル構造と完全に一致させます。

DROP FUNCTION IF EXISTS public.get_consultations_with_next_action(integer, integer);

CREATE FUNCTION public.get_consultations_with_next_action(
    page_limit integer,
    page_offset integer
)
RETURNS TABLE (
