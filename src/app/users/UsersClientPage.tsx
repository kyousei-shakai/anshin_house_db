// src/app/users/UsersClientPage.tsx (新規作成)
...
export default function UsersClientPage({ initialUsers, fetchError }: UsersClientPageProps) {
  const [users] = useState<User[]>(initialUsers) // データはpropsで受け取る
  const [searchTerm, setSearchTerm] = useState('')
  const [sortBy, setSortBy] = useState<'name' | 'created_at' | 'uid'>('name')

  // ★★★ このコンポーネントは、propsで渡されたデータを元に、
  // クライアントサイドでのフィルタリングとソートのみを担当します。
  const filteredAndSortedUsers = users
    .filter(user =>
      user.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      user.uid.toLowerCase().includes(searchTerm.toLowerCase()) ||
      (user.property_address?.toLowerCase().includes(searchTerm.toLowerCase()) ?? false) ||
      (user.property_name?.toLowerCase().includes(searchTerm.toLowerCase()) ?? false)
    )
    .sort((a, b) => {
      switch (sortBy) {
        case 'name':
